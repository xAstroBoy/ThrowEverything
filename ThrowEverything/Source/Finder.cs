...
static class Finder {
    ...
    internal static GameObject? Find(string path, bool dontWarn = true) {
        if (path.IsNotNullOrEmptyOrWhiteSpace()) {
            ...
            if (Root == null) {
                ...
                if (Root == null) {
                    if(!dontWarn) Plugin.Logger.LogDebug($"[ERROR (Find) ]  Gameobject on path [ {path} ]  is Invalid, No Root Object Found!");
                    return null;
                }
            }
            ...
            if (result == null) {
                if (!dontWarn) Plugin.Logger.LogDebug($"[ERROR (Find) ]  Gameobject on path [ {path} ]  is Invalid, No Child Object Found!");
                return null;
            }
            return result;
        }
    ...
    public static List<T>? GetRootGameObjectsComponents<T>(bool includeInactive = true) where T : Component {
        try {
            ...
            Plugin.Logger.LogDebug("Error parsing Components from Root Objects");
    ...
    internal static GameObject? FindRootSceneObject(string name, bool dontWarn = true) {
        ...
        if(!dontWarn) Plugin.Logger.LogDebug($"[WARNING (FindRootSceneObject) ]  Root Gameobject name [ {name} ]  is Invalid, No Object Found!");
        return null;
    }
    ...
    internal static GameObject? FindObject(this GameObject gameobject, string path, bool dontWarn = false) {
        ...
        if (obj == null) {
            if (!dontWarn) {
                Plugin.Logger.LogDebug($"[WARNING (FindObject) ]  Transform {gameobject.name} Doesnt have a object in path [ {path} ] !");
            }
            return null;
        }
    ...
    internal static Transform? FindObject(this Transform transform, string path, bool dontWarn = false) {
        ...
        if (obj == null) {
            if (!dontWarn) {
                Plugin.Logger.LogDebug($"[WARNING (FindObject) ]  Transform {transform.name} Doesnt have a object in path [ {path} ] !");
            }
            return null;
        }
